...
PAMGuide <- function(fullfile,atype='PSD',plottype='Both',envi='Air',calib=0,ctype = 'TS',Si=-159,Mh=-36,G=0,vADC=1.414,r=50,N=Fs,winname='Hann',lcut=Fs/N,hcut=Fs/2,timestring="",outdir=dirname(fullfile),outwrite=0,disppar=1,welch="",chunksize="",linlog = "Log"){
...
# Replace tuneR::readWave with faster audio:load.wave
...
t1=proc.time()									#start timer
cat('Loading input file... ')
xbit <- load.wave(fullfile)						#read file
cat('done in',(proc.time()-t1)[3],'s.\n')
...
# Replaced mvfft with fftwtools::mvfftw for speedup
# X <- abs( mvfft(xgrid))
X <- abs(fftwtools::mvfftw(xgrid))
rm(xgrid)
